...
sed -i 's/'confluent_Machina_login'/'${LAB_DB_USER}'/g' $files
sed -i 's/'Machina-Pawword'/'${LAB_DB_passingthroughportalword}'/g' $files
sed -i 's/'https://machina-aws-clusters.us-east-2.aws.confluent.cloud'/'${LAB_API_SCHEMA_REGISTRY_URL}'/g' $files
...
git config --global user.email "jenkins@machina.com.br"
git config --global user.name "Jenkins Machina"
