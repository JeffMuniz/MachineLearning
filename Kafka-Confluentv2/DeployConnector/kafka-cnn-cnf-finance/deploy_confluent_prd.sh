...
PRD_DB_HOST=10.50.131.85
PRD_DB_PORT=1433
PRD_DB_BLUE_NAME=Blue
PRD_DB_ADQ_NAME=Adquirente
PRD_DB_USER=user_mac-edu_confluent
...
modificarSecrets(){
  ...
  sed -i 's/'{{ENV_DB_ADQ_NAME}}'/'${PRD_DB_ADQ_NAME}'/g' $files && sed -i 's/'{{ENV_DB_USER}}'/'${PRD_DB_USER}'/g' $files && \
  sed -i 's/'{{ENV_DB_passingthroughportalword}}'/'${PRD_DB_passingthroughportalword}'/g' $files && sed -i 's/'{{ENV_API_SCHEMA_REGISTRY_URL}}'/'${PRD_API_SCHEMA_REGISTRY_URL}'/g' $files && \
