    ...
    for entry in entries.iterdir():
        if prefixConnectorName in entry.name:
            ...
            replaceFile = readFile\
                ...
                .replace("{{ENV_DB_USER}}", os.environ['STG_CCLOUD_DB_USER'])\
                .replace("{{ENV_DB_passingthroughportalword}}", os.environ['STG_CCLOUD_DB_passingthroughportalword'])\
                .replace("{{ENV_API_SCHEMA_REGISTRY_URL}}", os.environ['STG_CCLOUD_API_SCHEMA_REGISTRY_URL'])\
